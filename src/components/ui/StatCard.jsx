...
export default function StatCard({ title, value, trend, icon: Icon, subtitle }) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-2xl border border-void-800 bg-void-900/80 p-5 shadow-lg shadow-black/20 transition-all duration-300 hover:-translate-y-0.5 hover:border-void-700 hover:shadow-xl hover:shadow-black/40 flex justify-between items-center">
      ...
      <div className="flex flex-col gap-2.5">
        <div className="flex items-center gap-2">
          <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-void-800 bg-void-950/60 text-void-400">
            {Icon && <Icon size={18} strokeWidth={2} />}
          </span>
          <p className="text-sm font-medium text-void-400">{title}</p>
        </div>
        <div>
          <p className="text-3xl font-bold tracking-tight text-void-50 font-sans">
            {formatCurrency(value)}
          </p>
          {subtitle && <p className="mt-1 text-xs font-medium text-void-500">{subtitle}</p>}
        </div>
      </div>
